...
} from "../redux/api/projectDetailApi";
...
export default function Tasks() {
  ...
  const { data: dropdownOptions } = useGetDropdownOptionsQuery();
  ...
  const calculateRockDetails = (taskList) => {
    ...
    const maxHours = Math.max(
      ...taskList.map((task) => Number(task.hours || 0)),
      0
    );
    const totalResources = taskList.reduce(
      (sum, task) => sum + Number(task.resources || 0),
      0
    );
  ...
  const addTask = () => {
    if (newTask.title && newTask.department && newTask.comment) {
      ...
      const { totalHours, totalResources, rockSize, useCase } =
        calculateRockDetails(updatedTasks);
        ...
        <div className="rounded-xl md:rounded-2xl shadow-lg md:shadow-xl p-4 md:p-6 hover:shadow-xl md:hover:shadow-2xl transition-all bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="text-left">
              ...
              <div className="text-2xl md:text-3xl font-bold text-[#00CCFF] dark:text-white">
                {currentProject?.reports?.totalHours || 0}
              </div>
            </div>
            <div className="text-left">
              <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 uppercase">
              ...
              <div className="text-2xl md:text-3xl font-bold text-[#00CCFF] dark:text-white">
                {currentProject?.reports?.totalResources || 0}
              </div>
            </div>
          </div>
            ...
            <table className="w-full min-w-[650px]">
              <thead className="dark:bg-gray-700 bg-[#003399] text-white">
                <tr className="text-left dark:text-gray-300 text-xs sm:text-sm uppercase font-semibold">
                  <th className="px-4 sm:px-6 py-3 md:py-4 rounded-tl-xl">
                    Project Tasks
                  </th>
                  ...
                  <th className="px-4 sm:px-6 py-3 md:py-4 rounded-tr-xl text-center">
                    Actions
                  </th>
                </tr>
              </thead>
